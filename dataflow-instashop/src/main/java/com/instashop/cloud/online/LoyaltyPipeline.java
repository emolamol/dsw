...
import com.instashop.cloud.online.dofn.PubSubMessageToTableRow;
import com.instashop.cloud.online.schema.BigQueryDeadletterSchema;
 ...
 * _loyalty_event_history and _loyalty_tier_history. If any record fails
...
public class LoyaltyPipeline {
	...
	public static interface MyOptions extends DataflowPipelineOptions, StreamingOptions {
		...
		@Required
		@Default.String("projects/namita-186919/subscriptions/instashop_subscription")
	...
	public static void main(String[] args) {
		...
		String table_instashopLoyalty_event_history = options.getProject() + ":" + options.getLoyaltyInstanceId()
				+ ".instashop_loyalty_event_history";
		String table_instashop_loyalty_tier_history = options.getProject() + ":" + options.getLoyaltyInstanceId()
				+ ".instashop_loyalty_tier_history";
		String table_instashop_deadletter_table = options.getProject() + ":" + options.getLoyaltyInstanceId()
				+ ".deadletter_table";

		// Build the table schema for the output table -
		// instashop_loyalty_event_history
		List<TableFieldSchema> table_instashopLoyalty_event_history_fields = new ArrayList<>();
table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("retailstoreid").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("workstationid").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("businessdaydate").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("operatorname").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("itemcount").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("tender_total").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("tender_type").setType("STRING"));
		table_instashopLoyalty_event_history_fields
				.add(new TableFieldSchema().setName("transaction_discount_amount").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("taxtotal").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("barcode").setType("STRING"));
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("processedTime").setType("TIMESTAMP"));
		...
		table_instashopLoyalty_event_history_fields.add(new TableFieldSchema().setName("line_items").setType("STRUCT")
				.setMode("REPEATED").setFields(lineItemFields));
		TableSchema table_instashopLoyalty_event_history_fields_schema = new TableSchema()
				.setFields(table_instashopLoyalty_event_history_fields);

		// Build the table schema for the output table - table_instashop_loyalty_tier_history
		List<TableFieldSchema> table_instashop_loyalty_tier_history_fields = new ArrayList<>();
		table_instashop_loyalty_tier_history_fields.add(new TableFieldSchema().setName("party_id").setType("STRING"));
		table_instashop_loyalty_tier_history_fields.add(new TableFieldSchema().setName("loyalty_acct_id").setType("STRING"));
		table_instashop_loyalty_tier_history_fields
		...
		TableSchema table_instashop_loyalty_tier_history_schema = new TableSchema()
				.setFields(table_instashop_loyalty_tier_history_fields);
		...
		PCollectionTuple parsedPOSLTransaction = p //
				.apply("DesktopJSONFromPubSub",
						PubsubIO.readMessagesWithAttributes().withIdAttribute(PUB_SUB_ID_ATTR)
								.fromSubscription(options.getSubscription())) //
				.apply("ExtractJSONLogData", ParDo.of(new DoFn<PubsubMessage, LoyaltyInfo>() {
					@ProcessElement
		...
		parsedPOSLTransaction.get(DEADLETTER_TAG).setCoder(PubsubMessageWithAttributesCoder.of())
				.apply("FailedToTableRows", ParDo.of(new PubSubMessageToTableRow())).apply("WriteDeadletterToBigQuery",
						BigQueryIO.writeTableRows().to(table_instashop_deadletter_table).withSchema(deadletterSchema)
		...
		parsedLoyaltyEngineOutput.apply("EventHistoryRow", ParDo.of(new DoFn<LoyaltyInfo, TableRow>() {
			...
			public void processElement(ProcessContext c) throws Exception {
				...
				.apply("EventHistoryTable", BigQueryIO.writeTableRows().to(table_instashopLoyalty_event_history)//
						.withSchema(table_instashopLoyalty_event_history_fields_schema)//
		...
		parsedLoyaltyEngineOutput.apply("TierHistoryRow", ParDo.of(new DoFn<LoyaltyInfo, TableRow>() {
			@ProcessElement
			public void processElement(ProcessContext c) throws Exception {
				TableRow instashop_loyalty_tier_history_row = new TableRow();

				LoyaltyInfo info = c.element();
				instashop_loyalty_tier_history_row.set("party_id", info.getParty_id());
				instashop_loyalty_tier_history_row.set("loyalty_acct_id", info.getLoyalty_acct_id());
				instashop_loyalty_tier_history_row.set("loyalty_member_status", info.getLoyalty_member_status());
				instashop_loyalty_tier_history_row.set("loyalty_member_current_tier", info.getLoyalty_member_current_tier());
				instashop_loyalty_tier_history_row.set("loyalty_member_previous_tier",
						info.getLoyalty_member_previous_tier());
				instashop_loyalty_tier_history_row.set("loyalty_event_type", info.getLoyalty_event_type());
				instashop_loyalty_tier_history_row.set("loyalty_event_time", info.getLoyalty_event_time());
				c.output(instashop_loyalty_tier_history_row);
			}
		})) //
				.apply("TierHistoryTable", BigQueryIO.writeTableRows().to(table_instashop_loyalty_tier_history)//
						.withSchema(table_instashop_loyalty_tier_history_schema)//
						.withWriteDisposition(BigQueryIO.Write.WriteDisposition.WRITE_APPEND)
						.withCreateDisposition(BigQueryIO.Write.CreateDisposition.CREATE_IF_NEEDED));
